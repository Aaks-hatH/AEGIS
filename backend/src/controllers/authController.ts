...
export async function login(req: Request, res: Response) {
  ...
  const token = signToken(user as any); res.cookie("aegis_token", token, { httpOnly: true, sameSite: env.cookieSecure ? "none" : "lax", secure: env.cookieSecure, maxAge: 2 * 60 * 60 * 1000 });
...
export async function logout(req: Request, res: Response) { await audit(req, "user_logout", "session", req.user?.id); res.clearCookie("aegis_token", { httpOnly: true, sameSite: env.cookieSecure ? "none" : "lax", secure: env.cookieSecure }); return ok(res, { loggedOut: true }); }
