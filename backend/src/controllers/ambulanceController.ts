...
export async function analyzeReport(req: Request, res: Response) { const report = await AmbulanceReport.findById(req.params.id); if (!report) throw new HttpError(404, "Ambulance report not found"); report.analysis = analyzeAmbulance({ reportText: report.reportText, structuredSymptoms: report.structuredSymptoms, age: report.age, vitals: report.vitals as any, etaMinutes: report.etaMinutes }) as any; await report.save(); await audit(req, "ambulance_report_analyzed", "ambulance", String(report._id)); return ok(res, report); }
